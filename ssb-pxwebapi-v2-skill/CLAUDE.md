...
- For example URLs/queries, hit the live API and confirm HTTP 200 (or just run `check_examples.py`). Codelist prefixes are easy to forget: `agg_KommFylker` requires `F-` codes, `agg_KommSummer` requires `K-` codes (and usually `outputValues[Region]=aggregated`). A 400 response usually means a missing prefix or a missing required variable (`Tid`/`ContentsCode` are never eliminable).
